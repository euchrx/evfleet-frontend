...
const manualSteps = [
  {
    ...
    steps: "Inclua placa, marca, modelo, ano, tipo, combustível, capacidade e status. Depois vincule filial para manter custos e relatórios corretos.",
  },
  {
    title: "2. Vincule motoristas e veículos",
    route: "Caminho: Motoristas > + Cadastrar motorista",
    steps: "Cadastre dados de CNH e status. Mantenha apenas perfis ativos para evitar vínculos incorretos em abastecimentos, viagens e multas.",
  },
  {
    title: "3. Registre abastecimentos com KM",
    route: "Caminho: Abastecimentos > + Registrar",
    steps: "Informe veículo, motorista, filial, litros, valor e odômetro. O sistema calcula consumo médio e aponta anomalias automaticamente.",
  },
  {
    title: "4. Planeje manutenção preventiva",
    route: "Caminho: Manutenções > Planos de manutenção",
    steps: "Crie planos por KM ou tempo e configure alertas. Use o módulo de pneus para leituras técnicas e controle de desgaste.",
  },
  {
    title: "5. Controle débitos e documentos",
    route: "Caminho: Débitos e Multas / Gestão de Documentos",
    steps: "Registre vencimentos, status e responsáveis. Monitore as notificações para itens a vencer e pendências vencidas.",
  },
  {
    title: "6. Acompanhe indicadores no dashboard",
    route: "Caminho: Dashboard",
    steps: "Use os cards e rankings para decidir com rapidez: custo total, custos por categoria, motoristas e veículos com maior impacto financeiro.",
  },
];
    ...
    <div className="space-y-6">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <form onSubmit={handleAddVideo} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              <textarea rows={2} value={form.description} onChange={(event) => handleChange("description", event.target.value)} className="mt-1 w-full rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-orange-500 focus:ring-2 focus:ring-orange-200" placeholder="Resumo do conteúdo do vídeo" />
            </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por título, categoria ou descrição" className="w-full max-w-md rounded-xl border border-slate-300 px-4 py-3 outline-none transition focus:border-orange-500 focus:ring-2 focus:ring-orange-200" />
        </div>
