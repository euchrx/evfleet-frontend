...
import {
  DriversTablesSection,
  type DriverFilterForm,
  type DriverSortBy,
} from "./DriversTablesSection";
...
export function DriversPage() {
  ...
  useEffect(() => {
    const nextFilters: DriverFilterForm = {
      branchId: selectedBranchId || "",
      name: "",
      cpf: "",
      cnh: "",
  ...
  function handleFilterChange<K extends keyof DriverFilterForm>(
    field: K,
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!payload.companyId) {
        setFieldErrors((prev) => ({
          ...prev,
          name: "Empresa não selecionada. Selecione uma empresa antes de cadastrar.",
        }));
        return;
      }
  ...
  const filteredDrivers = useMemo(() => {
    ...
    const nameTerms = splitCsv(appliedFilters.name).map((item) => item.toLowerCase());
    ...
    if (branchIds.length) {
      filtered = filtered.filter((driver) => {
        if (!driver.vehicleId || !driver.vehicle) return true;
    ...
    if (nameTerms.length) {
      filtered = filtered.filter((driver) =>
        nameTerms.some((name) => driver.name.toLowerCase().includes(name)),
      );
    }
    ...
    if (vehicleIds.length) {
      filtered = filtered.filter((driver) =>
        driver.vehicleId ? vehicleIds.includes(driver.vehicleId) : false,
      );
    }
  ...
  const selectedDraftNameIds = useMemo(
    () => splitCsv(draftFilters.name),
    [draftFilters.name],
  );

  const selectedDraftVehicleIds = useMemo(
    () => splitCsv(draftFilters.vehicleId),
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      <DriversTablesSection
        ...
        selectedDraftStatusIds={selectedDraftStatusIds}
        nameOptions={nameOptions}
        selectedDraftNameIds={selectedDraftNameIds}
        vehicleSelectOptions={vehicleSelectOptions}
        selectedDraftVehicleIds={selectedDraftVehicleIds}
        filteredDrivers={filteredDrivers}
        paginatedDrivers={paginatedDrivers}
        selectedDriverIds={selectedDriverIds}
        onToggleDriverSelection={toggleDriverSelection}
        onToggleSelectAllDriversOnPage={toggleSelectAllDriversOnPage}
        allDriversOnPageSelected={allDriversOnPageSelected}
        onSort={handleSort}
        getSortArrow={getSortArrow}
        driverDocumentCountByDriverId={driverDocumentCountByDriverId}
